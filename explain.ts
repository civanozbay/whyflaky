...
export async function explainCommand(options: ExplainOptions): Promise<void> {
  ...
  if (!options.file && !options.error) {
    ...
    console.log(
      chalk.gray(
        "   Example: testai explain --file src/auth.test.ts --error 'Cannot read property of undefined'"
      )
    );
    process.exit(1);
  }
  ...
  if (options.file) {
    ...
    try {
      ...
      spinner.succeed(
        `Read test file + ${context.importedFiles.length} imported file(s)`
      );
      ...
      const errorMessage =
        options.error || "The test is failing. Analyze the code for issues.";
