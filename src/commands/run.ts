...
export async function runCommand(options: RunOptions): Promise<void> {
  ...
  try {
    ...
    spinner.fail(
      `Could not read results: ${err instanceof Error ? err.message : err}`,
    );
    console.log(chalk.gray(`\nExpected results at: ${RESULTS_PATH}`));
    console.log(
      chalk.gray(
        "Make sure mochawesome is installed: npm install --save-dev mochawesome",
      ),
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < failedTests.length; i++) {
    ...
    console.log(
      chalk.red.bold(`  ❌ FAILED TEST ${i + 1}/${failedTests.length}`),
    );
  ...
  console.log(chalk.cyan.bold(`\n${"─".repeat(55)}`));
  console.log(
    chalk.cyan.bold(`  📊 Summary: ${failedTests.length} test(s) analyzed`),
  );
